function [out] = equi2face(im, x_degree, y_degree, z_degree) %x_degree->clock-wise
...
vfov = 100*pi/180; % Default vertical FOV(100)


% Define top left point of equirectangular in normalized co-ordinates
output_width = double(outsize);
output_height = double(outsize);
...
roll = z_degree;
    ...
    out = zeros([output_height, output_width, size(im,3)], class(im));
    ...
    for c = 1 : size(im, 3)
        out(:,:,c) = cast(reshape(out_pix(:,c), output_height, ...
